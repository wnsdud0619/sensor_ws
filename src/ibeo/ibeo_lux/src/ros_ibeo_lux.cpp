...
int main(int argc, char **argv)
{
  ...
  ros::Publisher ibeo_alive_check_pub = n.advertise<std_msgs::Bool>("/dgist/system/status/lidar/front", 1);
        ...
        if (!messages.empty())
        {
          for (unsigned int i = 0; i < messages.size(); i++)
          {
            ...
            if (class_parser != NULL && pub != pub_list.end())
            {
              ...
              std_msgs::Bool system_checker;
              system_checker.data = true;
              ibeo_alive_check_pub.publish(system_checker);
